...
| 3.1.3 | `known_through_contact_id` на карточке                        | Через кого познакомился                              | ❌     |
...
| 3.2.3 | `GET /api/v1/links` — все связи для графа                     | Визуализация полного графа                           | ❌     |
...
| 6.2 | Фильтр `relationship_type`                 | `?relationship_type=colleague`                         | ❌     |
| 6.3 | Сортировка `sort=last_contact_at`          | По дате последнего взаимодействия                      | ❌     |
| 6.4 | Фильтр `last_contact_before=N`             | С кем давно не общались (N дней)                       | ❌     |
| 6.5 | Фильтр `has_birthday_soon=N`               | ДР в ближайшие N дней                                  | ❌     |
| 6.6 | Полнотекстовый поиск (tsvector + GIN)      | `GET /api/v1/search?q=` по заметкам и интересам        | ❌     |
...
| #   | Задача                                              | Результат                               | Статус |
| --- | --------------------------------------------------- | --------------------------------------- | ------ |
| 8.1 | Интеграционные тесты ключевых эндпоинтов API        | contacts CRUD, auth, links, interactions| ❌     |
| 8.2 | Тесты сервисного слоя (агрегация обещаний и т.п.)   | Unit-тесты бизнес-логики                | ❌     |
...
| --------- | ---- | ---------------------------------------------- |
| 🔴 1      | 6.1  | Серверный поиск `?q=` — сейчас поиск ограничен 50 загруженными контактами |
| 🔴 2      | 8.1  | Интеграционные тесты API — нет ни одного теста |
| 🟡 3      | 6.4  | Фильтр "давно не общались" — органичен для текущего UI |
| 🟡 4      | 6.5  | Фильтр по дням рождения                        |
| 🟡 5      | 5.6  | Агент 2 «Concierge» (дизайн готов в AGENTS.md) |
| 🟢 6      | 7.1  | Теги для группировки контактов                 |
| 🟢 7      | 3.2.3| `GET /api/v1/links` для визуализации графа     |

...
class ContactCard(Base):
    ...
    id: Mapped[UUID] = mapped_column(
        ...
        primary_key=True,
        default=uuid4,
        doc="Уникальный идентификатор контакта",
    )
    ...
    socials: Mapped[dict | None] = mapped_column(
        JSON,
        default=dict,
        doc="Социальные сети и контакты в формате JSON",
    )

    first_met_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True),
        doc="Дата и время знакомства",
    )
    first_met_place: Mapped[str | None] = mapped_column(
        String(255),
        doc="Место знакомства",
    )
    first_met_context: Mapped[str | None] = mapped_column(
        Text,
        doc="Обстоятельства знакомства",
    )

    relationship_type: Mapped[str | None] = mapped_column(
        String(20),
        doc="Тип отношений: business/personal/other",
    )
    projects_notes: Mapped[str | None] = mapped_column(
        Text,
        doc="Проекты и договоренности",
    )
    ...
    last_contact_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True),
        doc="Дата последнего контакта",
    )
    last_contact_summary: Mapped[str | None] = mapped_column(
        Text,
        doc="Краткая заметка о последнем контакте",
    )

    promises: Mapped[list | None] = mapped_column(
        JSON,
        default=list,
        doc="Обещания и упоминания (список)",
    )
    recommendations: Mapped[str | None] = mapped_column(
        Text,
        doc="Рекомендации и мнения",
    )
    competence_rating: Mapped[int | None] = mapped_column(
        Integer,
        doc="Оценка компетенций (1-5)",
    )
    competence_notes: Mapped[str | None] = mapped_column(
        Text,
        doc="Комментарий к оценке компетенций",
    )

    goals: Mapped[list | None] = mapped_column(
        JSON,
        default=list,
        doc="Цели и амбиции (список)",
    )
    ambitions: Mapped[str | None] = mapped_column(
        Text,
        doc="Амбиции и планы",
    )

    family_members: Mapped[list["ContactFamilyMember"]] = relationship(
        "ContactFamilyMember",
...
class ContactFamilyMember(Base):
    ...
    contact_id: Mapped[UUID] = mapped_column(
        PG_UUID(as_uuid=True),
        ForeignKey("contact_cards.id", ondelete="CASCADE"),
        index=True,
        doc="Ссылка на контакт",
    )
    full_name: Mapped[str] = mapped_column(
        String(255),
        nullable=False,
    ...
    contact: Mapped["ContactCard"] = relationship(
        "ContactCard",
        back_populates="family_members",
        primaryjoin="ContactFamilyMember.contact_id == ContactCard.id",
    )
...
class ContactInteraction(Base):
    ...
    id: Mapped[UUID] = mapped_column(
        ...
        primary_key=True,
        default=uuid4,
        doc="Уникальный идентификатор взаимодействия",
    )
    contact_id: Mapped[UUID] = mapped_column(
        PG_UUID(as_uuid=True),
        ForeignKey("contact_cards.id", ondelete="CASCADE"),
    ...
    occurred_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
        nullable=False,
        doc="Дата и время взаимодействия",
    )
    channel: Mapped[str | None] = mapped_column(
        String(50),
        doc="Канал общения (встреча/звонок/сообщение)",
    )
    notes: Mapped[str | None] = mapped_column(Text, doc="Заметки о встрече")
    promises: Mapped[list | None] = mapped_column(
        JSON,
        default=list,
        doc="Обещания/упоминания (список)",
    )
    mentions: Mapped[list | None] = mapped_column(
        JSON,
        default=list,
        doc="Темы и упоминания (список)",
    )

    contact: Mapped["ContactCard"] = relationship(
        "ContactCard",
        back_populates="interactions",
        primaryjoin="ContactInteraction.contact_id == ContactCard.id",
    )
